...
class Preprocessor:
    ...
    def create_splits(self, norm_method = 'tanh_norm', features_path='X.p.gz', labels_path='labels.csv'):
        ...
        feature_group = np.array(feature_temp)
        ...
        # Add index to each feature name
        feature_origin = np.array([f"{name}_{i}" for i, name in enumerate(feature_origin)])
        ...
        if norm_method == "tanh_norm":     
            ...
            f_feature_origin = feature_origin[feat_filt]
            f_feature_group = feature_group[feat_filt]

            # Then normalize the other datasets using the same parameters and feat_filt
            X_val, _, _, _ = self.normalize(X_val, mean, std, mean2, std2, feat_filt=feat_filt, norm=norm_method)
            X_train, _, _, _ = self.normalize(X_train, mean, std, mean2, std2, feat_filt=feat_filt, norm=norm_method)
            X_test, _, _, _ = self.normalize(X_test, mean, std, mean2, std2, feat_filt=feat_filt, norm=norm_method)
        else:
        ...
        with gzip.open('test%dval%dnorm%s.p.gz' % (self.test_fold, self.val_fold, norm_method), 'wb') as f:
            pickle.dump((X_tr, X_val, X_train, X_test, y_tr, y_val, y_train, y_test, index_names, f_feature_origin, f_feature_group), f)
...
data = preprocessor.create_splits(norm_method='tanh')
