...
class WeBGNNConv(MessagePassing):
    ...
    def forward(self, x, edge_index):
        ...
        up_drug_edge_weight = torch.ones(self.num_drug_edge).to(device=self.device) # [/107*2=214]
        ...
        down_drug_edge_weight = torch.ones(self.num_drug_edge).to(device=self.device)
...
class WeBGNNDecoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, decoder_dim, node_num, num_edge, num_gene_edge, device):
        ...
        self.parameter1 = torch.nn.Parameter(torch.randn(int(embedding_dim*3), decoder_dim).to(device=self.device))
        self.parameter2 = torch.nn.Parameter(torch.randn(decoder_dim, decoder_dim).to(device=self.device))
    ...
    def forward(self, x, edge_index, drug_index):
        ...
        ypred = torch.zeros(batch_size, 1).to(device=self.device)
    ...
    def loss(self, pred, label):
        pred = pred.to(device=self.device)
